...
- 左上标识使用两行：`Figerpoint Detector` 使用 `--font-fangsong`、20px 标题；`by Ikaleio` 使用 `--font-heiti`、14px 正文。（来源：2026-09-22 用户要求。）
...
- 标识标题使用 `--font-fangsong`（FangSong、STFangsong 及仿宋/衬线回退）；署名使用 `--font-heiti`（Heiti SC、SimHei、STHeiti 及黑体回退）。
