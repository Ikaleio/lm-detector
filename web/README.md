...
`src/` 包含检测页面、只读样本库与中英文文案。检测复用 `../shared/`，正式数据来自 `../data/`。API 请求经同源 `/api/proxy` 转发到允许的 HTTPS 服务，上游不需要支持浏览器 CORS。代理实现位于 `../server/proxy.ts`，Vite 开发及预览服务复用该实现。
...
构建前的 `scripts/sync-data.ts` 仅同步脱敏数据，不依赖外层研究代码。静态产物为 `dist/`。Vercel 和 Cloudflare Pages 均从 monorepo 根目录部署。两平台的函数入口、允许的上游及运行命令见 [部署说明](../docs/deployment.md)。GitHub Pages 仅支持手动检测和参考库查看。
